...
class TextConfig():
    ...
    num_labels = 3  # number of labels (for unmerged dataset, use 8)
    ...
    num_epochs = 5000 #64  # epochs
    batch_size = 26  # batch_size
    print_per_batch = 200  # print result
    require_improvement = 1600  # stop training if no improvement over 1600 global_step

    output_dir = './result_clinicalBERT/'
    ...
    '''
    training_data = data_dir + 'train_decreased.txt'
    dev_data = data_dir + 'test_decreased.txt'
    test_data = data_dir + 'test_decreased.txt'
    '''

    BIOBERT_BASE_DIR = './pretrained_bert_model/pretrained_bert_tf/biobert_pretrain_output_all_notes_150000/'
    vocab_file = BIOBERT_BASE_DIR+'vocab.txt'  # the path of vocab file
    bert_config_file = BIOBERT_BASE_DIR+'bert_config.json'  # the path of clinical bert_cofig file
    init_checkpoint = BIOBERT_BASE_DIR + 'model.ckpt-150000'  # the path of clinical bert model
...
class Processor(object):
    ...
    def get_labels(self):
        """See base class."""
        return ['BEFORE', 'OVERLAP', 'AFTER'] #['ENDED_BY', 'BEFORE_OVERLAP', 'BEFORE', 'BEGUN_BY', 'DURING', 'OVERLAP', 'AFTER', 'SIMULTANEOUS']
...
def train():
    ...
    tf.compat.v1.logging.info("*****************Loading training data*****************")
    ...
    tf.compat.v1.logging.info("*****************Loading dev data*****************")
    ...
    tf.compat.v1.logging.info("Time cost: %.3f seconds...\n" %(time.time() - start_time))

    tf.compat.v1.logging.info("Building session and restore bert_model...\n")
    ...
    tf.compat.v1.logging.info('Training and evaluating...\n')
    best_acc = 0
    last_improved = 150000  # record global_step at best_val_accuracy
    flag = False

    for epoch in range(config.num_epochs):
        batch_train = batch_iter(trian_data, config.batch_size)
        start = time.time()
        tf.compat.v1.logging.info('Epoch:%d' % (epoch + 1))
        for batch_ids, batch_mask, batch_segment, batch_label in batch_train:
            ...
            _, global_step, train_summaries, train_loss, train_accuracy = session.run([model.optim, model.global_step, merged_summary, model.loss, model.acc], feed_dict=feed_dict)
            ...
            if global_step % config.print_per_batch == 0:
                ...
                if val_accuracy > best_acc: # merged_acc > best_acc
                    saver.save(session, save_path)
                    best_acc = val_accuracy # merged_acc
                    last_improved = global_step
                    improved_str = '*'
                else:
                    improved_str = ''
                tf.compat.v1.logging.info("step: {},train loss: {:.3f}, train accuracy: {:.3f}, val loss: {:.3f}, val accuracy: {:.3f},training speed: {:.3f}sec/batch {}".format(global_step, train_loss, train_accuracy, val_loss, val_accuracy, (end - start) / config.print_per_batch, improved_str))
                start = time.time()

            if global_step - last_improved > config.require_improvement:
                tf.compat.v1.logging.info("No optimization over 1600 steps, stop training")
                flag = True
                break
        if flag:
            break
...
def test():
    ...
    if not os.path.exists(save_dir):
        tf.compat.v1.logging.info("maybe you didn't train")
        exit()

    tf.compat.v1.logging.info("*****************Loading testing data*****************")
    ...
    tf.compat.v1.logging.info(msg.format(test_loss, test_accuracy))
    ...
    # evaluate
    tf.compat.v1.logging.info("Precision, Recall and F1-Score...")
    tf.compat.v1.logging.info(metrics.classification_report(y_test_cls, y_pred_cls, target_names=label_list))

    tf.compat.v1.logging.info("Confusion Matrix...")
    cm = metrics.confusion_matrix(y_test_cls, y_pred_cls)
    tf.compat.v1.logging.info(cm)
